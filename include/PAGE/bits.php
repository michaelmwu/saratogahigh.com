...
class bit
{
	...
	public function remove( &$int, $bit)
	{
		$int &= ~ (1 << (int) $bit);
	}
	
	public function toggle( &$int, $bit )
	{
		$int ^= 1 << (int) $bit;		
	}

	public function read($int, $bit)
	{
		return $int & (1 << (int) $bit);
	}
	
	public function set_range(&$int,$value,$length,$offset = 0)
	{
		$int &= ~bit::generate_mask($offset,$offset + $length);
		$int |= $value << $offset;
	}
	
	public function remove_range(&$int,$value,$length,$offset = 0)
	{
		$int &= ~bit::generate_mask($offset,$offset + $length);
	}
	
	public function toggle_range(&$int,$value,$length,$offset = 0)
	{
		$int ^= bit::generate_mask($offset,$offset + $length);
	}
	
	public function read_range($int,$value,$length,$offset = 0)
	{
		return ($int & ~bit::generate_mask($offset,$offset + $length)) >> $offset;
	}
	
	public function split_range($start,$end)
	{
	...
	public function generate_mask($start,$end)
	{
		$range = $end - $start + 1;
...
class bitmask
{
	...
	public function bit_split($bits,$offset)
	{
		$values = array();
	...
	public function set_value( $value, $length, $offset )
	{
		$i = 0;
	...
	public function remove_value( $start, $end )
	{
	...
	public function toggle_value( $start, $end )
	{
		$range = bit::split_range( $start, $end );
		foreach($range as $element)
		{
	...
	public function read_value( $start, $end )
	{
		$value = 0;
		$i = 0;
		$range = bit::split_range( $start, $end );
	...
	public function stringout() // Returns a string of your nice little bits
	{
		$string = "";
