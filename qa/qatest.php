	...
	if($qa = mysql_fetch_array($rsqa, MYSQL_ASSOC))
	{
		if($loggedin || $qa['QA_ALLOW_ANONYMOUS'])
		{
    		...
    		if($loggedin && ($_GET['fill'] == 'new' || ($qa['QA_AUTOSTART'] == 1 && !$isauthor)) && $qa['QA_OPEN'] == 1)
    		{
    			...
    			if($numfills['COUNT(*)'] < $qa['QA_FILLLIMIT'] || is_null($qa['QA_FILLLIMIT']))
    			{
					// Insert new QAFILL				
...
?>
		...
		if($isadmin || mysql_fetch_array($export_user, MYSQL_ASSOC)
		{
			...
			if($isadmin)
				print 'New Exporter:<form name="newexport" action="qa.php?id=' . $qa['QA_ID'] . '" method="POST"><input type="text" name="export" onClick="loadSearchID();" size="6"><input type="hidden" name="action" value="newexport"><input type="submit" value="Go"></form>';
		}
		...
		while($listqa = mysql_fetch_array($rsallqas, MYSQL_ASSOC))
		{
			...
			if(mysql_num_rows($rsallfills) == 0)
				print '<p style="margin: 0 0 0 1em">not started</p>';
...
function loadSearchID()
{
	...
	sframe.src='/directory/search-id.php?form=addexport&formelement=export';
