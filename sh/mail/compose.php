...
// mail/compose.php: composes messages and verifies their recipients
...
function addrecipient($msgid, $recipient, $sendtype, $threadno)
{
	$crec = mysql_query('SELECT COUNT(*) AS COUNT FROM MAILREC_LIST WHERE MAILREC_MSG=' . $msgid . ' AND MAILREC_RECIPIENT=' . $recipient);
	$l = mysql_fetch_array($crec, MYSQL_ASSOC);
	mysql_free_result($crec);
	
	if($l['COUNT'] == 0)
	{
		mysql_query('INSERT INTO MAILREC_LIST (MAILREC_THREAD, MAILREC_MSG, MAILREC_RECIPIENT, MAILREC_SENDTYPE) VALUES (' . $threadno . ', ' . $msgid . ', ' . $recipient . ', "' . $sendtype . '")');
		return true;
	}
	else
		return false;
...
function parsereclist($xto, $msgid, $sendtype, $threadno)
{
	$x = 0;
	
	global $errorm, $duplist;

	if(strlen($xto) > 0)
	{
		$recsto = explode(',', $xto);
		foreach($recsto as $citem)
		{
			$wherestr = '';
			$restriction = '';
			if(eregi('^([a-z\' -]+ ?):([0-9]+|f|p|s)$', $citem, $matches))
			{
				$citem = supertrim($matches[1]);
				$restriction = $matches[2];
				
				if(is_numeric($restriction))
					$wherestr = ' AND USER_GR=' . $matches[2];
				else if(strtolower($restriction) == 's')
					$wherestr = ' AND USER_GR>1';
				else if(strtolower($restriction) == 'f')
					$wherestr = ' AND USER_GR=0';
				else if(strtolower($restriction) == 'p')
					$wherestr = ' AND USER_GR=1';
			}
			else
			{
				$citem = supertrim($citem);
			}

			$cnamedigest = namedigest($citem);

			if(is_numeric($_POST['au' . $cnamedigest]))
			{
				$currecord = mysql_query('SELECT USER_FULLNAME, USER_ID, USER_GR, USER_VALIDATED FROM USER_LIST WHERE USER_ID=' . $_POST['au' . $cnamedigest] . ' AND USER_FULLNAME="' . addslashes($citem) . '"') or die("Query A1 failed.");
			}
			else
			{
				$currecord = mysql_query('SELECT USER_FULLNAME, USER_ID, USER_GR, USER_VALIDATED FROM USER_LIST WHERE USER_FULLNAME="' . addslashes($citem) . '"' . $wherestr) or die("Query A0 failed.");
			}
			
			if(mysql_num_rows($currecord) == 1)
			{
				$l = mysql_fetch_array($currecord, MYSQL_ASSOC);
				if($l['USER_VALIDATED'] == 1)
				{
					if(addrecipient($msgid, $l['USER_ID'], $sendtype, $threadno))
						$x++;
				}
				else
					$errorm = 'Sorry, that user (' . $l['USER_FULLNAME'] . ') hasn\'t verified his/her account. He/she must do so before you can exchange messages.';
			}
			else if(mysql_num_rows($currecord) > 1)
			{
				while($l = mysql_fetch_array($currecord, MYSQL_ASSOC))
					$duplist .= '<tr><td><input type="radio" name="au' . $cnamedigest . '" value="' . $l['USER_ID'] . '"></td><td><a href="/directory/?id=' . $l['USER_ID'] . '">' . htmlentities($l['USER_FULLNAME']) . ', ' . GradePrint($l['USER_GR']) .  '</a></td></tr>';
				
				$errorm = 'One or more of the names you entered were ambiguous. Please select the correct recipient(s) from the list below.';
			}
			else
			{
				$errorstr = 'We couldn\'t find ';
				if(is_numeric($restriction))
					$errorstr .= 'anyone in the Class of ' . $restriction;
				else if(strtolower($restriction) == 's')
					$errorstr .= 'any students';
				else if(strtolower($restriction) == 'f')
					$errorstr .= 'any faculty';
				else if(strtolower($restriction) == 'p')
					$errorstr .= 'any parents';
				else
					$errorstr .= 'anyone';
				$errorstr .= ' named ' . supertrim($citem) . '. Please check your spelling and try again.';
				
				$errorm = $errorstr;
			}
			mysql_free_result($currecord);
		}
	}
