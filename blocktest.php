...
{
	$userid = 101;
    mysql_query("UPDATE FRONTPREF_LIST SET FRONTPREF_VALUE=" . quote_smart($value) . " WHERE FRONTPREF_USER='" . quote_smart($userid) . "' AND FRONTPREF_BOX=" . quote_smart($box_id) . " AND FRONTPREF_KEY=" . quote_smart($key));
}
...
function box_header(&$box) // Print out the title part of the box.
{
    ...
    $result = mysql_query("SELECT FRONTBOX_ID FROM FRONTBOX_LIST WHERE FRONTBOX_ID='" . $box->id . "'");
    
	$row = mysql_fetch_array($result, MYSQL_ASSOC);

    $box_prefs=get_box_prefs($row['FRONTBOX_ID'], 101);
    $content_state=$box_prefs['content_state'];
    
$content = '<h2 class="' . $box->h2class . '" onMouseOver="startMoveCursor(event);" onMouseDown="startDrag(event);">';
	if($href = $box->h2href)
		$content .= '<a href="' . $href . '">';
	$content .= $box->title;
	if($href)
    ...
    $content .= '</h2><span class="xbox">';
    if($content_state=="expanded")
            $content .= '<a href="#" class="expand" onClick="return expandMe(event);" style="display: none">&darr;</a><a href="#" class="minimize" onClick="return minimizeMe(event);">&uarr;</a>';
    else
            $content .= '<a href="#" class="expand" onClick="return expandMe(event);">&darr;</a><a href="#" class="minimize" onClick="return minimizeMe(event);" style="display: none">&uarr;</a>';
          
	    $content .= ' <a href="#" onClick="return removeMe(event);">&times;</a></span>';
...
function rss_block(&$box,$label,$url,$href = "")
{
	...
	$box->content =	rss_content($url,5,false,false);
}
	...
	*/
	...
	if ($loc_id) // Location code selected - Display detail info. A specific city has been selected from the drop down menu. Get forecast.
	{
		...
		$content .= "<span style=\"font-size: 10px;\">(Last updated $last_update).</span><br>\n"
			. "<p><img style=\"border: 1px solid #000000; margin: 10px; float: left;\" src=\"images/weather/$curr_icon.png\" alt=\"$curr_text\">"
...
function blog_block(&$box)
{
	...
	$box->script = '<script type="text/javascript">
	<!--
