...
if($_POST['sf'] == 'true')
{
	if($uline = db::get_prefix_row('USER', "USER_FN='" . $_POST["xfn"] . "' AND
											USER_LN='" . $_POST["xln"] . "' AND
											USER_ACTIVATION='" . $_POST['code'] . "' AND
											USER_UNAME IS NOT NULL") ) // Already registered people are redirected to reset password.
		header("Location: http://" . DNAME . "/resetpw.php?un=" . $uline['USER_UNAME'] . "&code=" . $_POST['code']);
}
...
function checkinfo($xfn,$xln,$code)
{
	...
	if(strlen($code) > 0)
	{
		if(!$line = db::get_prefix_row('USER',"USER_FN='" . addslashes($xfn) . "' AND
			USER_LN='" . addslashes($xln) . "' AND
		...
		if(strlen($line['USER_UNAME']) > 0)
			return "You seem to already have registered. Submit this form to reset your password.";
	}
	else
...
{
	if(!email::is_valid($email,EMAIL_SYNTAX))
...
<p style="font-size: medium">Your information will be checked dynamically after you fill in or finish any changes to the fields.</p>
